...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "number_of_prints",
            type=int,
            help="Pass a number in to indicate the number of prints to be added to the database",
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        for print in range(number_of_prints):
            save = PrintTracker(
                print_name=random_name,
                total_hours_printed=total_hours_printed,
                total_minutes_printed=total_minutes_printed,
                material_used="PLA",
                color="Yellow",
            )
